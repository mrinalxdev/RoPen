...
def top_k_top_p_filtering(logits, top_k=0, top_p=0.0, filter_value=-float('Inf')):
    ...
    if top_k > 0:
        # claude : remove all tokens with a probability less than the last token of the top-k
...
def generate(model, tokenizer, prompt="", max_new_tokens=500, temperature=0.8, top_k=40, top_p=0.9):
    ...
    for _ in range(max_new_tokens):
        ...
        logits = logits[:, -1, :] / temperature  # Now shape [1, vocab_size]

        # 🔴 Fix: Squeeze to 1D before filtering
        logits = logits.squeeze(0)  # Now shape [vocab_size]
        logits = top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p)
        logits = logits.unsqueeze(0)  # Back to [1, vocab_size]
